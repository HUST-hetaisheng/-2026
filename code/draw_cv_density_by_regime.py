"""
方案3：CV 和 CI-Width 密度曲线（按 Regime 分层）
验证：不确定性在不同参赛者之间是否保持一致
- 上图：CV 密度曲线
- 下图：CI-Width 密度曲线
...
for regime in ['rank', 'percent', 'bottom2']:
    ...
    print(f"  - CV均值: {data.mean():.4f}")
    print(f"  - CV标准差: {data.std():.4f}")
    print(f"  - CV的变异系数 (一致性指标): {cv_of_cv:.4f}")
