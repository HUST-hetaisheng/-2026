...
To compare these results with Percent seasons, we map the inferred discrete ranks $r^F_{i,w}$ to continuous shares $f_{i,w}$ using a Softmax distribution:
\begin{equation}
    f_{i,w} = \frac{\exp(-\gamma \cdot (r^F_{i,w} - 1))}{\sum_k \exp(-\gamma \cdot (r^F_{k,w} - 1))}
    \label{eq:gibbs_softmax}
\end{equation}
where $\gamma > 0$ controls the steepness of the vote-share decay across ranks. This parameter is calibrated using early Percent-era seasons where both rank and percentage data are available.
...
Since the rank-based combined score is discrete and non-differentiable, we employ a \textbf{Greedy Swapping Heuristic}  to find a feasible fan ranking $\mathbf{r}^F_w$. 
...
\subsubsection{Probabilistic Judges' Save and Ensemble Inference}
The Judges' Save mechanism introduces an additional layer of uncertainty: even after determining the Bottom-Two, the final elimination depends on judges' discretion. We model this decision as a score-biased logistic choice: given Bottom-Two members $\{a, b\}$, the probability of eliminating $a$ is
\begin{equation}
    P(\text{Elim } a \mid a, b \in \mathcal{B}) = \frac{1}{1 + \exp\left(\beta(J_a - J_b)\right)}
    \label{eq:judge_save_prob}
\end{equation}
where $\beta > 0$ quantifies judges' tendency to save higher-scored contestants. The optimal fan ranking $\mathbf{r}^{F*}_w$ maximizes the likelihood of the observed elimination: $\mathbf{r}^{F*}_w = \arg\max P(\text{Elim } e_w \mid \mathcal{B}(\mathbf{r}^F))$ subject to $e_w \in \mathcal{B}(\mathbf{r}^F)$.

To quantify the inherent ambiguity, we employ an \textbf{Ensemble Perturbation Method}. By injecting Gaussian noise $\epsilon \sim \mathcal{N}(0, \sigma^2)$ into judge scores and running the optimization $K=1{,}000$ times, we generate a posterior distribution over fan rankings. These discrete rankings are then converted to continuous vote shares using the Gibbs-Softmax mapping in Eq.~\eqref{eq:gibbs_softmax}. The variance of this ensemble (Coefficient of Variation) serves as our formal measure of model certainty, reflecting the ``transparency'' of each elimination event.
