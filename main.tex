...
\subsubsection{Analysis of Model Consistency}
The \textbf{Model Consistency Rate (MCR)}, defined as the frequency with which the reconstructed fan vote distribution $\mathbf{v}_t$ satisfies the hard elimination constraints within the feasible region $\Omega_t$, serves as a primary validator of our framework's logical integrity. 

As illustrated in the Figure 1, the model achieves near-perfect consistency ($>98\%$) throughout the \textit{Percent Regime} (Seasons 3–27). This suggests that the continuous nature of percentage-based rules provides sufficient "gradient information" for the SLSQP optimizer to converge on a valid MAP estimate. 
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/consistency_by_season.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
However, a slight decrease in consistency is observed in the \textit{Judges' Save Regime} (Seasons 28–34), particularly in mid-competition weeks. This marginal drop is not a failure of the algorithm, but rather a mathematical reflection of \textbf{Information Asymmetry}: because the show does not always disclose the identity of the second contestant in the "Bottom Two," the model occasionally faces an underdetermined constraint set. Despite this, the sustained high consistency across diverse week-types—including multiple eliminations and finals—demonstrates that our regime-adaptive approach effectively captures the underlying mechanics of the voting system.

\subsubsection{Quantitative Assessment of Model Certainty}
We evaluate the \textbf{Certainty} of our fan vote estimates through the dual lenses of the \textbf{Coefficient of Variation (CV)} and the \textbf{Confidence Interval (CI) Width} across the longitudinal dataset.

In our framework, \textit{Certainty} is inversely proportional to the dispersion of the ensemble of valid solutions:
\begin{itemize}
    \item \textbf{High Certainty in Percent Regime:} The model exhibits remarkably low CV values (averaging $<0.05$) and narrow CI ribbons. This high precision stems from the fact that percentage-based scores impose strict geometric boundaries on the probability simplex, leaving little room for alternative vote-share scenarios.
    
    \item \textbf{Transition to Lower Certainty in Rank Regime:} Conversely, the \textbf{Ensemble Distribution Plot} (Figure \ref{fig:uncertainty_plot}) reveals significantly wider CI widths and elevated CVs (approaching $0.15$–$0.22$) during Seasons 1–2. This phenomenon is a direct consequence of \textbf{Ordinal Compression}: since ranks only provide the relative order and discard the magnitude of fan support, a vast "feasible volume" of numeric vote shares can lead to the same observed ranking.
\end{itemize}

The model's ability to quantify this certainty shift proves that our uncertainty quantification module correctly identifies the "transparency" of different show formats, proving that the move from Rank to Percent regimes in Season 3 fundamentally increased the mathematical determinism of the competition.
